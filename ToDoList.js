...
function showPerticularCategory(newCategory, index) {
  document.getElementById("categoryData").innerHTML +=
    `<tr><td><input type="button" id="${newCategory.title}" value="${newCategory.title}" class="categoryFormButtons" onClick="showTodoList(${newCategory.title})"></td><td>` +
  ...
  for (let i = 0; i < categoryArray.length; i++) {
    if (categoryArray[i].title == categoryTitle.value) {
      for (let j = 0; j < categoryArray[i].todoList.length; j++) {
        showPerticularTodo(
          categoryArray[i].todoList[j],
          j,
          categoryArray[i].title
        );
      }
    }
  }
